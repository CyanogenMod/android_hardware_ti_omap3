...
FMC_STATIC void HandleRdsSetEnableInt(void)
{
    ...
    FMC_CORE_SendWriteCommand(FMC_FW_OPCODE_CMN_INT_MASK_SET_GET, getMask());
}
...
{   
    ...
    /* Disable FR and enable the default interrupts or disable all interupts acorrding to the queue */
    FMC_CORE_SendWriteCommand(FMC_FW_OPCODE_CMN_INT_MASK_SET_GET, getMask());
...
FMC_STATIC void HandleSetAFStart(void)
{
    ...
    /* Enable the interrupts  acorrding to the queue*/
    FMC_CORE_SendWriteCommand(FMC_FW_OPCODE_CMN_INT_MASK_SET_GET,  getMask());
}
FMC_STATIC void HandleSetAfFinish(void)
{
...
FMC_STATIC void HandleCompleteScanMain(void)
{
    ...
    if(_fmRxSmData.callReason == FM_RX_SM_CALL_REASON_START) 
    {
        _fmRxSmData.completeScanOp.isStopCompleteScanRequest=FMC_FALSE;
  ...
  /* If an upper event was received update the handler to stop seek */
   ...
   /*If we got Stop Complete Scan in the middle of the operation change the handler to the correct one*/ 
...
FmRxStatus _FM_RX_SM_AllocateCmd(   FmRxContext     *context,
                                            ...
                                            FmcBaseCmd      **cmd)
{
    FmTxStatus      status = FM_RX_STATUS_SUCCESS;
    ...
    {
        FMC_LOG_ERROR(("FM RX: Too many pending Commands. (%s)", FMC_DEBUG_FmRxCmdStr(cmdType)));
        FMC_RET(FMC_STATUS_TOO_MANY_PENDING_CMDS);
    }
    else if (status != FMC_STATUS_SUCCESS)
    {
        FMC_FATAL(FMC_STATUS_INTERNAL_ERROR, ("_FM_RX_AllocateCmdAndAddToQueue: FMC_POOL_Allocate Failed (%s)", 
...
FmRxStatus _FM_RX_SM_FreeCmd(FmcBaseCmd **cmd)
{
    ...
    FMC_VERIFY_FATAL((status == FM_RX_STATUS_SUCCESS), FM_RX_STATUS_INTERNAL_ERROR,
                        ("FM_RX_SM_FreeCmd: FMC_POOL_Free Failed (%s) for %s", 
                        FMC_DEBUG_FmcStatusStr(status), FMC_DEBUG_FmRxCmdStr(cmdType)));
...
{
    return (FMC_U16)(gain*FMC_FW_RX_FM_GAIN_STEP);
