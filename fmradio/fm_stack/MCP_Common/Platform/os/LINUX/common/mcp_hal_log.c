...
static McpU8 readConfFile()
{
           ...
           (strcmp(a_inputBuffer, "\n") != RES_OK) )
    {
            ...
            (strcmp(p_token, LOGGER_IP) == RES_OK) )
       {
           ...
           LOGD(" readConfFile: p_token = %s \n", p_token);
		   if (strlen(p_token) < sizeof(gMcpLogUdpTargetAddress))
		      strcpy(gMcpLogUdpTargetAddress, p_token);
		   else
		   {
            ...
            (strcmp(p_token, LOGGER_PORT) == RES_OK) )
       {
           ...
           LOGD(" readConfFile: p_token = %s \n", p_token);
			...
			gMcpLogUdpTargetPort = atoi(p_token);

			LOGD("PORT VALUE READ: %l",  gMcpLogUdpTargetPort);
    ...
    fclose(fp);
    LOGD(" readConfFile: Exiting Successfully. \n");
    return retVal;
}
