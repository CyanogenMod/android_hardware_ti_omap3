													...
													McpHalOsSemaphoreHandle 	*semaphoreHandle)
{
	...
	*semaphoreHandle = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t));
	FM_ASSERT(*semaphoreHandle != NULL);
	...
	if(rc != 0)
	{
		FM_TRACE("MCP_HAL_OS_CreateSemaphore | pthread_mutexattr_init() failed: %s", strerror(rc));
	...
	if(rc != 0)
	{
		FM_TRACE("MCP_HAL_OS_CreateSemaphore | pthread_mutexattr_settype() failed: %s", strerror(rc));
	...
	if(rc != 0)
	{
		FM_TRACE("MCP_HAL_OS_CreateSemaphore | pthread_mutex_init() failed: %s", strerror(rc));
		pthread_mutexattr_destroy(&mutex_attr);
		return MCP_HAL_OS_STATUS_FAILED;
	}
	...
	if(rc != 0)
	{
		FM_TRACE("MCP_HAL_OS_CreateSemaphore | pthread_mutexattr_destroy() failed: %s", strerror(rc));
...
McpHalOsStatus MCP_HAL_OS_DestroySemaphore(McpHalOsSemaphoreHandle *semaphoreHandle)
{
	...
	FM_TRACE("MCP_HAL_OS_DestroySemaphore: killing semaphore");
	
	rc = pthread_mutex_destroy(*semaphoreHandle);
	*semaphoreHandle = NULL;
	if (rc != 0)
	{
		FM_TRACE("MCP_OS_DestroySemaphore | pthread_mutex_destroy() failed: %s", strerror(rc));
		return MCP_HAL_OS_STATUS_FAILED;
	}
													...
													McpHalOsTimeInMs 		timeout)
{
	...
	{
		FM_ASSERT("finite locking time not supported yet");
	}
	
	rc = pthread_mutex_lock(semaphoreHandle);
	if (rc != 0)
	{
		FM_TRACE("MCP_HAL_OS_LockSemaphore: pthread_mutex_lock() failed: %s", strerror(rc));
		return MCP_HAL_OS_STATUS_FAILED;
	}
...
McpHalOsStatus MCP_HAL_OS_UnlockSemaphore(McpHalOsSemaphoreHandle semaphoreHandle)
{
	...
	if(rc != 0)
	{
		FM_TRACE("MCP_HAL_OS_UnlockSemaphore: pthread_mutex_unlock() failed: %s", strerror(rc));
		return MCP_HAL_OS_STATUS_FAILED;
	}
...
#endif
	FM_TRACE("BTHAL_OS_Sleep | usleep() failed: %s", strerror(errno));
...
McpHalOsTimeInMs MCP_HAL_OS_GetSystemTime(void)
{
	...
	FM_TRACE("MCP_HAL_OS_GetSystemTime: gettimeofday() failed: %s", strerror(errno));
	FM_ASSERT(0);
}
