...
void MCP_LoadMngr_Init(void)
{
    ...
    /*Init the excution complete CB function */
...
void MCP_LoadMngr_Create(BtHciIfObj *hciIfObj)
{
    ...
    /*Crete State Machine */
    MCP_GenSM_Create (&(tDataGenSm) , MCP_LM_STATE_MAX_NUM, MCP_LM_EVENT_MAX_NUM, &(tSmMatrix[0][0]),MCP_LM_STATE_UNLOADED,MCP_LM_EventToString,MCP_LM_StateToString);
...
McpHalStatus MCP_LoadMngr_SetScriptName (const char *scriptName, const char* scriptLocation)
{
    ...
    pScriptLocation = (char *)scriptLocation;
...
McpHalStatus MCP_LoadMngr_LoadScript (TLoadMngrCB fCB, void *pUserData)
{
    ...
    /* check for perliminary conditions that there is a valid CB */
...
McpHalStatus MCP_LoadMngr_StopLoadScript (TLoadMngrCB fCB, void *pUserData)
{
    ...
    /* check for perliminary conditions that there is a CB */
    MCP_VERIFY_ERR ((NULL != fCB), MCP_HAL_STATUS_FAILED,
                    ("MCP_LoadMngr_LoadScript: NULL CB!"));
...
MCP_STATIC void MCP_LM_Load (void *pUserData)
{
    ...
    MCP_HAL_MISC_Sprintf (scriptFullFileName, "%s%s", pScriptLocation, pScriptName);
    tScriptLocation.locationData.fullFileName = scriptFullFileName;

    /* Excute the requested script*/
    ...
    else if (MCP_BTS_SP_STATUS_FILE_NOT_FOUND == status)
    {
        ...
        memInitScriptFound =  MCP_RomScriptsGetMemInitScriptData(
                                    ...
                                    &tScriptLocation.locationData.memoryData.address);

        /* verify the script was found in memory */
        MCP_VERIFY_FATAL ((memInitScriptFound == MCP_TRUE), MCP_HAL_STATUS_FAILED,
...
/*---------------------------------------------------------------------------
 *            Call back function for script excution complete
 *---------------------------------------------------------------------------
*/
void MCP_LM_ExcutionCompleteCB(McpBtsSpContext *context, McpBtsSpStatus status)
{
    ...
    {
        /* Abort was completed succesfully send anMCP_LM_EVENT_ABORT_DONE to the SM */
        MCP_GenSM_Event (&tDataGenSm, MCP_LM_EVENT_ABORT_DONE, NULL);
    }

    else if (status==MCP_BTS_SP_STATUS_SUCCESS)
    {
        /* Load script was successfull send an event to SM that the load is done */
