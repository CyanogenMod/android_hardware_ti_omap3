...
MCP_STATIC McpU8 Set_params_Fm_Pcm_I2s_mode_6450(Cal_Resource_Data  *presourcedata,McpU8 *pBuffer)
{
    ...
    pcmiLeftRightSwap = (McpU16)fmI2sConfigParams[FM_PCMI_RIGHT_LEFT_SWAP_LOC].value;


    params |= pcmiLeftRightSwap<<FM_PCMI_RIGHT_LEFT_SWAP_OFFSET;

    bitOffsetVector = (McpU16)fmI2sConfigParams[FM_PCMI_BIT_OFFSET_VECTOR_LOC].value ;


    params |= bitOffsetVector<<FM_PCMI_BIT_OFFSET_VECTOR_OFFSET;

    slotOffsetVector = (McpU16)fmI2sConfigParams[FM_PCMI_SLOT_OFSET_VECTOR_LOC].value ;   


    params |= slotOffsetVector<<FM_PCMI_SLOT_OFSET_VECTOR_OFFSET;

    pcmInterfaceChannelDataSize =(McpU16) fmI2sConfigParams[FM_PCMI_PCM_INTERFACE_CHANNEL_DATA_SIZE_LOC].value ;
