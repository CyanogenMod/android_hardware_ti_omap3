...
MCP_STATIC void _CCM_IM_InitObjData(CcmImObj *this, McpHalChipId chipId);
...
CcmImStatus CCM_IM_Create(McpHalChipId chipId, CcmImObj **this,  CcmChipOnNotificationCb notificationCB, void *userData)
{
    _CcmImStatus    status;
    ...
    *this = &_ccmIm_StaticData.imObjs[chipId];

    _CCM_IM_InitObjData(*this, chipId);

    /* Generate a name for the mutex of this instance (one per chip) */
    _CCM_IM_GenerateMutexName(chipId, mutexName);
    
    halOsStatus = MCP_HAL_OS_CreateSemaphore(mutexName, &((*this)->mutexHandle));
    ...
    status = _CCM_IM_BtTranMngr_Create((*this)->chipId, _CCM_IM_BtTranMngrCb, (*this)->mutexHandle, &((*this)->tranMngrObj));
    ...
    (*this)->chipOnNotificationCB = notificationCB;
...
CcmImStatus CCM_IM_Destroy(CcmImObj **this)
{
    ...
    status = _CCM_IM_BtTranMngr_Destroy(&((*this)->tranMngrObj));
    MCP_VERIFY_FATAL((status == _CCM_IM_STATUS_SUCCESS), _CCM_IM_STATUS_INTERNAL_ERROR,
                        ("CCM_IM_Create: _CCM_IM_BtTranMngr_Destroy Failed"));
    
    halOsStatus = MCP_HAL_OS_DestroySemaphore(&((*this)->mutexHandle));
    MCP_VERIFY_FATAL((halOsStatus == MCP_HAL_OS_STATUS_SUCCESS), CCM_IM_STATUS_INTERNAL_ERROR,
                                        ...
                                        CCM_IM_StackHandle  *stackHandle)
{
    ...
    MCP_VERIFY_FATAL((this != NULL), CCM_IM_STATUS_INVALID_PARM, ("CCM_IM_RegisterClient: Null this"));
    ...
    stackData = &this->stacksData[stackId];

    stackData->containingImObj = this;
...
void _CCM_IM_InitObjData(CcmImObj *this, McpHalChipId chipId)
{
    _CcmIm_SmStackIdx   stackIdx;
    
    this->chipId = chipId;
    this->mutexHandle = MCP_HAL_OS_INVALID_SEMAPHORE_HANDLE;
    this->tranMngrObj = NULL;
    this->chipOnNotificationCalled = MCP_FALSE;

    for (stackIdx  = 0; stackIdx < CCM_IM_MAX_NUM_OF_STACKS; ++stackIdx)
    {
        _CCM_IM_InitStackData(&this->stacksData[stackIdx]);
    }
}
...
_CcmImStatus _CCM_IM_StackSmHandler_OnComplete(_CcmImStackData *stackData)
{
    ...
    this = stackData->containingImObj;

    /*if this is the first ON notification */
    if (MCP_FALSE == this->chipOnNotificationCalled)
    {
        this->chipOnNotificationCalled = MCP_TRUE;
        /* notify other CCM modules, with the chip version */
        _CCM_IM_BtTranMngr_GetChipVersion (this->tranMngrObj, &projectType, &versionMajor, &versionMinor);
        this->chipOnNotificationCB (this->userData, projectType, versionMajor, versionMinor);
    }
...
{
    MCP_HAL_MISC_Sprintf(mutexName, "%s%d", CCM_IM_CONFIG_MUTEX_NAME, chipId);
}
