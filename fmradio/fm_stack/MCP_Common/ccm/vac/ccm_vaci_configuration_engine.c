                                                    ...
                                                    McpConfigParser 			*pConfigParser)
{
    ...
    *this = &(tCEObjects[ chipId ]);

    /* initialize operations data */
    for (uIndex = 0; uIndex < CAL_OPERATION_MAX_NUM; uIndex++)
    {
        (*this)->tOperations[ uIndex ].bSupported = MCP_FALSE;
        for (uIndex2 = 0; uIndex2 < CCM_VAC_MAX_NUMBER_OF_CLIENTS_PER_OPERATION; uIndex2++)
        {
            (*this)->tOperations[ uIndex ].fCB[ uIndex2 ] = NULL;
        }
        (*this)->tOperations[ uIndex ].uNumberOfCBs = 0;
        (*this)->tOperations[ uIndex ].eOperationState = CCM_VAC_OPERATION_STATE_IDLE;
        (*this)->tOperations[ uIndex ].tCurrentRequiredResources.uNumOfResources = 0;
        (*this)->tOperations[ uIndex ].uResourceBeingConfigured = 0;
        (*this)->tOperations[ uIndex ].tCALUserData.ptConfigEngine = *this;
        (*this)->tOperations[ uIndex ].tCALUserData.eOperation = uIndex;
    }

    /* store the CAL object */
    (*this)->pCAL = pCAL;

    (*this)->pConfigParser = pConfigParser;

     /* Initialize the semaphore name */
    MCP_HAL_MISC_Sprintf (mutexName, "%s%d", CCM_VAC_MUTEX_NAME, chipId);

    /* create the semaphore */
    eOsStatus = MCP_HAL_OS_CreateSemaphore (mutexName, &((*this)->tSemaphore));
    ...
    status = _CCM_VAC_MappingEngine_Create (chipId, (*this)->pConfigParser, &((*this)->ptMappingEngine));
    ...
    status = _CCM_VAC_AllocationEngine_Create (chipId, (*this)->pConfigParser, &((*this)->ptAllocationEngine));
    MCP_VERIFY_ERR ((CCM_VAC_STATUS_SUCCESS == status), CCM_VAC_STATUS_FAILURE_UNSPECIFIED,
...
ECCM_VAC_Status _CCM_VAC_ConfigurationEngine_Configure (TCCM_VAC_ConfigurationEngine *this)
{
    ...
    CAL_GetAudioCapabilities (this->pCAL,
                              _CCM_VAC_MappingEngine_GetConfigData (this->ptMappingEngine),
                              _CCM_VAC_AllocationEngine_GetConfigData (this->ptAllocationEngine),
                              &tAvailResources, &tAvailOperations);

    /* configure allowed operations: */
    /* first, get number or operations from ini file */
    eCPStatus = MCP_CONFIG_PARSER_GetIntegerKey (this->pConfigParser, 
    ...
    for (uIndex = 0; uIndex < uOperationNum; uIndex++)
    {
        ...
        eCPStatus = MCP_CONFIG_PARSER_GetAsciiKey (this->pConfigParser, 
                                                   (McpU8 *)CCM_VAC_CONFIG_INI_OPERATIONS_SECTION_NAME,
        ...
        this->tOperations[ eOperation ].bSupported = MCP_TRUE;
    }

    /* configure the mapping engine with chip capabilities */
    status = _CCM_VAC_MappingEngine_Configure (this->ptMappingEngine, 
                                               &tAvailResources, &tAvailOperations);
    ...
    status = _CCM_VAC_AllocationEngine_Configure (this->ptAllocationEngine, 
                                                  &tAvailResources, &tAvailOperations);
    MCP_VERIFY_ERR ((CCM_VAC_STATUS_SUCCESS == status), status,
...
void _CCM_VAC_ConfigurationEngine_Destroy (TCCM_VAC_ConfigurationEngine **this)
{
    ...
    eOsStatus = MCP_HAL_OS_DestroySemaphore(&((*this)->tSemaphore));
    ...
    _CCM_VAC_MappingEngine_Destroy (&((*this)->ptMappingEngine));

    /* destroy the allocation engine */
    _CCM_VAC_AllocationEngine_Destroy (&((*this)->ptAllocationEngine));
