                            ...
                            const char *taskName)
{
	...
	*/
	...
	if(0 != rc)
	{
		FMC_LOG_INFO(("FMHAL_OS_CreateTask | pthread_create() failed to create 2nd FM thread: %s", strerror(errno)));
		goto err;
	}

	FMC_LOG_INFO(("Created 2nd FM thread, id = %u", threadHandle));
...
static void *fm_wait_for_interrupt_thread(void *dev)
{
	...
	if (setsockopt(dd, SOL_HCI, HCI_FILTER, &nf, sizeof(nf)) < 0) {
		FMC_LOG_ERROR(("failed to set socket options"));
		ret = FMC_STATUS_FAILED;
		goto close;
	}
	...
	while (fmParams2.taskRunning) {
		...
		while ((n = poll(&p, 1, 500)) == -1) {
			if (errno == EAGAIN || errno == EINTR)
				continue;
			FMC_LOG_ERROR(("failed to poll socket"));
			ret = FMC_STATUS_FAILED;
			goto close;
		}
		...
		while ((len = read(dd, buf, sizeof(buf))) < 0) {
			if (errno == EAGAIN || errno == EINTR)
				continue;
			FMC_LOG_ERROR(("failed to read socket"));
			ret = FMC_STATUS_FAILED;
			goto close;
		}
