...
#define DEFAULTDSPEXEC  "./strmcopy.dof55l"
...
static const struct DSP_UUID nodeuuid = {
	0x7eeb2c7e, 0x785a, 0x11d4, 0xa6, 0x50, {
	...
	if (DSP_SUCCEEDED(status)) {
		...
		if (DSP_SUCCEEDED(status)) {
			...
			if (DSP_SUCCEEDED(status)) {
				...
				/* Run task. */
				status =
				    RunTask(&strmcopyTask, inFile, outFile);
				if (DSP_SUCCEEDED(status)) {
...
static DSP_STATUS InitializeProcessor(struct STRMCOPY_TASK *copyTask)
{
	...
	/* Attach to IVA */
	/* status = DSPProcessor_Attach(DEFAULTIVAUNIT, NULL,*/
...
static DSP_STATUS InitializeNode(struct STRMCOPY_TASK *copyTask)
{
	...
	if (DSP_SUCCEEDED(status)) {
		...
		if (DSP_SUCCEEDED(status)) {
			...
			fprintf(stdout,
				"DSPNode_Allocate failed. Status = 0x%x\n",
				(UINT) status);
		}
	}
	...
	if (DSP_SUCCEEDED(status)) {
		status =
		    DSPNode_Connect((DSP_HNODE) DSP_HGPPNODE, 0,
				    copyTask->hNode, 0, &attrs);
		if (DSP_FAILED(status)) {
			fprintf(stdout,
				"DSPNode_Connect failed. Status = 0x%x\n",
				(UINT) status);
		}
	}
	...
	if (DSP_SUCCEEDED(status)) {
		status =
		    DSPNode_Connect(copyTask->hNode, 0,
				    (DSP_HNODE) DSP_HGPPNODE, 0, &attrs);
		if (DSP_FAILED(status)) {
			fprintf(stdout,
				"DSPNode_Connect failed. Status = 0x%x\n",
				(UINT) status);
	...
	if (DSP_SUCCEEDED(status)) {
		...
		if (DSP_SUCCEEDED(status)) {
			...
			fprintf(stdout,
				"DSPNode_Create failed. Status = 0x%x\n",
				(UINT) status);
		}
	}
...
static DSP_STATUS InitializeStreams(struct STRMCOPY_TASK *copyTask)
{
	...
	if (DSP_SUCCEEDED(status)) {
		...
		if (DSP_SUCCEEDED(status)) {
			fprintf(stdout,"DSPStream_AllocateBuffers for "
												"input stream succeeded.\n");
		} else {
	...
	if (copyTask->ppOutBufs && copyTask->ppInBufs) {
		...
		do {
			...
			if (cBytesRead > 0) {
				...
				/* Put a buffer into the input channel to get back buffer
				 * sent.*/
				...
				fprintf(stdout, "Writing %x bytes to output file.\n",
...
static DSP_STATUS CleanupStreams(struct STRMCOPY_TASK *copyTask)
{
	...
	if (copyTask->hInStream) {
		...
		if (copyTask->ppInBufs) {
			...
			if (DSP_FAILED(status)) {
				fprintf(stdout,
					"DSPStream_Close of input stream failed. "
					"Status = 0x%x\n", (UINT) status);
			}
