...
static DSP_STATUS InitializeNode(struct SCALE_TASK *scaleTask)
{
	...
	if (DSP_SUCCEEDED(status)) {
		status = DSPNode_Allocate(scaleTask->hProcessor,&uuid,NULL,&nodeAttrIn,
															&scaleTask->hNode);
		if (DSP_SUCCEEDED(status)) {
	...
	if (DSP_SUCCEEDED(status)) {
		status = DSPNode_Connect((DSP_HNODE)DSP_HGPPNODE,0,scaleTask->hNode,0,
																		&attrs);
		if (DSP_FAILED(status)) {
	...
	if (DSP_SUCCEEDED(status)) {
		status = DSPNode_Connect(scaleTask->hNode,0,(DSP_HNODE) DSP_HGPPNODE,0,
																		&attrs);
		if (DSP_FAILED(status)) {
			fprintf(stdout, "DSPNode_Connect failed. Status = 0x%x\n",
																(UINT)status);
...
static DSP_STATUS InitializeStreams(struct SCALE_TASK *scaleTask)
{
	...
	if (DSP_SUCCEEDED(status)) {
		status = DSPStream_AllocateBuffers(scaleTask->hOutStream,DEFAULTBUFSIZE,
														&scaleTask->pOutBuf, 1);
		if (DSP_SUCCEEDED(status)) {
